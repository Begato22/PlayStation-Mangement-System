...
import 'package:playstation/shared/componentes/constant.dart';
...
class RoomsPage extends StatelessWidget {
              ...
              children: [
                ...
                Stack(
                  ...
                  children: [
                    ...
                    CircleAvatar(
                      radius: 5,
                      backgroundColor: cubit.rooms[index].isBusy
                          ? Colors.green
                          : Colors.grey,
                    ),
                  ],
                  ...
                  onTap: () async {
                    if (cubit.rooms[index].isBusy) {
                      showDialogAlert(
                        context: context,
                        message:
                            'Your Room is working now please wait until it finish its time and then you can remove it.',
                        url: 'note',
                        needOk: false,
                      );
                    } else {
                      showDialogAlert(
                        ...
                        url: 'sure',
                        onOk: () =>
                            cubit.deleteRoomFromDatabase(cubit.rooms[index].id),
                        needCancel: true,
                      );
                    }
                  },
            ...
            child: roomCardButton(
              ...
              onPressed: () {
                if (cubit.rooms[index].isBusy) {
                  ...
                  showDialog(
                    context: context,
                    builder: (ctx) => BlocConsumer<PlaystationHomeCubit,
                        PlaystationHomeStates>(
                      listener: (ctx, state) {},
                      builder: (ctx, state) {
                        return buildInsertSessionFeildBox(
                          cubit,
                          index,
                          durationController,
                          ctx,
              ...
              duration: cubit.rooms[index].isBusy
                  ? cubit.getDateTimeOfSpesificSession(cubit.rooms[index].id)
                  : null,
              sessionModel: cubit.rooms[index].isBusy
                  ? cubit.getSpesificSession(cubit.rooms[index].id)
                  : null,
              cubit: cubit,
            ),
          ),
              ...
              child: GestureDetector(
                onTap: () async {
                  showDialogAlert(
                    context: context,
                    message:
                        "Are you sure you want to Finsh Time of room (${cubit.rooms[index].name}) ?",
                    url: 'sleep',
                    needCancel: true,
                    onOk: () async {
                      return await cubit.finishTime(
                          cubit, context, cubit.rooms[index].id);
                    },
                  );
                },
                child: Row(
  ...
  AlertDialog buildInsertSessionFeildBox(
    ...
    return AlertDialog(
      ...
      actions: <Widget>[
        TextButton(
          onPressed: () async {
            if (cubit.isChecked) {
              print('cubit.isChecked : ${cubit.isChecked}');
              ...
              await cubit.updateStatusRoomDatabase(
                  'true', cubit.rooms[index].id);
              ...
              if (formKey.currentState!.validate()) {
                if (double.parse(durationController.text) * 60 > 8 * 60) {
                  showToast(
                      '${durationController.text} hours is too many, some thig wrong!',
                      ToastState.warning);
                } else {
                  if (double.parse(durationController.text) * 60 < 0) {
                    showToast('${durationController.text} hours is too short!',
                        ToastState.warning);
                  } else {
                    double duration =
