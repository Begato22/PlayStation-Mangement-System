...
import '../../../Modules/navigation bar screens/account.dart';
import '../../../Modules/navigation bar screens/home.dart';
import '../../../Modules/navigation bar screens/online.dart';
import '../../../Modules/navigation bar screens/rooms.dart';
...
class PlaystationHomeCubit extends Cubit<PlaystationHomeStates> {
  ...
  bool isHidden = true;
  IconData passwordVisabilityIcon = Icons.visibility;
  void changeVisibility() {
    isHidden = !isHidden;
    if (isHidden) {
      passwordVisabilityIcon = Icons.visibility;
    } else {
      passwordVisabilityIcon = Icons.visibility_off;
    }
...
//********************************** Bottom Sheet **********************************
      ...
      onCreate: (Database database, int version) async {
        ...
        try {
          await database.execute(
              'CREATE TABLE Room (id INTEGER PRIMARY KEY, name TEXT, roomType TEXT, hourPrice INTEGER, isBusy TEXT)');
  ...
  Future<dynamic> insertNewRoomIntoDatabase({
    ...
    return await database.transaction(
      (txn) async {
        try {
          await txn.rawInsert(
              'INSERT INTO Room(name, roomType, hourPrice, isBusy) VALUES("$name", "$roomType", $hourPrice, "false")');
  ...
  Future<void> getLastRoomFromDatabase(database) async {
    try {
      var value = await database
          .rawQuery('select * from Room ORDER BY id DESC LIMIT 1');
  ...
  Future<void> getLastHistoryFromDatabase(database) async {
    try {
      var value = await database
          .rawQuery('select * from History ORDER BY id DESC LIMIT 1');
      for (var element in value) {
        history.add(
          HistoryModel(
            id: element['id'],
...
//     );
  ...
  Future<void> updateStatusRoomDatabase(String isBusy, int id) async {
    await database
        .rawUpdate('UPDATE Room SET isBusy = ? WHERE id = ?', [isBusy, id]);
    getSpesificRoom(id).isBusy = parseBool(isBusy);
    print("@@ object = ${getSpesificRoom(id).isBusy}");
    emit(UpdateStatus());
  }
  ...
  Future<void> deleteDatabase() =>
      databaseFactory.deleteDatabase('playstation.db').then(
        (value) {
          // print("DB Deleted.");
        },
      );
  ...
  DateTime getDateTimeOfSpesificSession(int roomId) {
    SessionModel session =
        online.firstWhere((element) => element.roomId == roomId);
  ...
  RoomModel getSpesificRoom(int roomId) {
    return rooms.firstWhere((element) => element.id == roomId);
  }

  SessionModel getSpesificSession(int roomId) {
  ...
  Future<void> finishTime(
    ...
    SessionModel session = getSpesificSession(roomId);
    ...
    double totalHours =
        (duration.hour * 60 + duration.minute + duration.second / 60) / 60;
    double totalCost = session.sessionType.contains('Single')
        ? totalHours * getSpesificRoom(session.roomId).hourPrice
        : totalHours * getSpesificRoom(session.roomId).hourPrice * 1.75;
    await updateStatusRoomDatabase('false', getSpesificRoom(session.roomId).id);
    await database.rawUpdate('UPDATE Session SET end_time = ? WHERE id = ?',
        [endTime.toString(), session.id]);
    print(" ^^ UPDATE Session");
    await database.rawUpdate('UPDATE Session SET total_cost = ? WHERE id = ?',
        [totalCost, session.id]);
    print(" ^^ UPDATE Session");
